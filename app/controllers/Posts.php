...
class Posts extends Controller
{
    ...
    {
        $posts = $this->postModel->findPostByTopic($id);
        if ($posts) {
            $data = [
                'posts' => $posts,
            ];
    ...
    public function create()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (empty($data['titleError']) && empty($data['summaryError']) && empty($data['bodyError']) && empty($data['imageError'])) {
